 ...
 *
 * <p>A term table is a list of terms present in the <a
 * href=http://wikipedia.org/wiki/Algebraic_normal_form>algebraic normal form (ANF)</a>
 * representation of a Boolean function. In particular, an individual term is product of some or all
 * of a function's arguments with 1. As examples, consider the term "x0 * x1 * x3" and the ANF
 * expression "x0 + x0 x2 + x1 x2 x3 + 1".
 *
...
public interface TermTable {
   ...
   *
   * <p>Note that a term table may have more arguments than appear in its representation. For
   * example, "x0 + 1" can still be treated as a function of x0, x1, and x2.
   *
   ...
   * The lowest order bit in the vector indicates the presence of the first argument "x0", and the
   * remaining bits refer to the remaining consecutive arguments. The term in question is the
   ...
   *
   * <p>Each bit vector in the list returned indicates the presence of a single algebraic term. The
   * lowest-order bit in a vector indicates that it contains the first argument "x0", and the
   * remaining bits refer to the remaining consecutive arguments. The term in question is the
   * product of 1 and all present arguments.
   *
   * <p>E.g. 01101 = "x0 * x2 * x3"
   ...
   * <p>The constant term 1 is represented by a vector of zeros.
   *
   * @return A list of bit vectors (each of length {@code getArity()} representing the terms present
   *     in this table.
   */
   ...
   *
   * <p>The resulting truth table corresponds to the values of this term table evaluated for every
   * possible argument value. Note that this operation is expensive - it runs in n-squared time,
   * where n is the length of the truth table.
   *
